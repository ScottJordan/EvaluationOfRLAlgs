...
```
`-a` The name of the algorithm to run, e.g., `-a sarsa-parl2`. See [algconstructors.jl](experiments/algconstructors.jl) for a list of all names.  
`-e` Path to a JSON containing the information about the environment to run. See [envconf/](experiments/envconf) for examples.  
...
This example and other variations are provide in the file [demo.jl](experiments/demo.jl) along with a simple way to display the distribution of performance for an algorithm.
